	...
	_p[56] = {
		value: function(require, exports, module) {
			...
			Module.register("PriorityModule", function() {
				var minder = this;
				var PRIORITY_COLORS = [null, ["#FF1200", "#840023"],
					["#0074FF", "#01467F"],
					["#00AF00", "#006300"],
					["#FF962E", "#B25000"],
					["#A464FF", "#4720C4"],
	...
	_p[66] = {
		value: function(require, exports, module) {
			...
			function xhrLoadImage(info, callback) {
				return Promise(function(resolve, reject) {
					...
					xmlHttp.onerror = function() {
						let name = info.url.split('?')[0].split('/').pop();
						let ext = '.' + name.split('.').pop();
						name = name.length < 24 ? name : (name.substr(0, 5) + '...' + name.substr(-5 - ext.length))
						toastr.warning(name + "由于跨域请求失败！建议导出json文件！或者使用其他工具截图", '异常提示', {
			...
			function getSVGInfo(minder) {
				var paper = minder.getPaper(),
					paperTransform, domContainer = paper.container,
					svgXml, svgContainer, svgDom, renderContainer = minder.getRenderContainer(),
				...
				function dealWithPath(d, dealWithPattern) {
					...
					for(var i = 0, l = d.length; i < l; i++) {
						switch (d[i]) {
							...
							case "z":
								{
									pattern.push(cache.join(""), d[i]);dealWithPattern(pattern);strArr.push(pattern.join(""));cache = [];pattern = [];
									break;
								}
							case ".":
							case "e":
								...
								if(d) {
									d = dealWithPath(d, function(pattern) {
										switch (pattern[0]) {
											...
											case "T":
												{
													pattern[1] = +pattern[1] - parentX;pattern[3] = +pattern[3] - parentY;
													break;
												}
											case "Q":
											case "S":
												{
													pattern[1] = +pattern[1] - parentX;pattern[3] = +pattern[3] - parentY;pattern[5] = +pattern[5] - parentX;pattern[7] = +pattern[7] - parentY;
													break;
												}
											case "A":
												{
													pattern[11] = +pattern[11] - parentX;pattern[13] = +pattern[13] - parentY;
													break;
												}
											case "C":
												{
													pattern[1] = +pattern[1] - parentX;pattern[3] = +pattern[3] - parentY;pattern[5] = +pattern[5] - parentX;pattern[7] = +pattern[7] - parentY;pattern[9] = +pattern[9] - parentX;pattern[11] = +pattern[11] - parentY;
												}
										}
									});
